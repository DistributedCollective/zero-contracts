...
contract("BorrowerOperations", async accounts => {
  ...
  let massetManager;
  let nueMockToken;
  let nueMockTokenTransferWithPermit;
  ...
  const testCorpus = ({ withProxy = false }) => {
    before(async () => {
      ...
      nueMockTokenTransferWithPermit = await NueMockTokenTransferWithPermit.at(nueMockTokenTransferWithPermitAddress);
    ...
    it("repayZusdFromDLLR(): repays partial ZUSD debt - decreases user's ZUSDToken balance by correct amount", async () => {
      ...
      const permission = await signERC2612Permit(alice_signer, nueMockToken.address, alice_signer.address, nueMockTokenTransferWithPermit.address, decreaseAmount.toString());
    ...
    it("closeNueTrove(): reduces a Trove's debt to zero", async () => {
      ...
      const permission = await signERC2612Permit(alice_signer, nueMockToken.address, alice_signer.address, nueMockTokenTransferWithPermit.address, value.toString());
